...
export default function ClientAddTeamMemberForm() {
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      <FileUploadField name="photo" label="Passport-size photo" accept="image/jpeg,image/png" />
      ...
      <div>
        ...
        <input
          ...
          style={{ width: '100%', padding: '10px 14px', borderRadius: '8px', border: '1px solid var(--line)', background: '#fff', fontSize: '13.5px', color: 'var(--ink)' }}
        />
        <p style={{ fontSize: '11.5px', color: 'var(--slate)', marginTop: '6px' }}>Lower numbers show first. Members with the same number are sorted by when they were added.</p>
      </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px', marginTop: '12px' }}>
        ...
        >
          {loading ? <><span className="btn-spinner" />Saving...</> : 'Add team member'}
        </button>
      </div>
    </form>
  );
