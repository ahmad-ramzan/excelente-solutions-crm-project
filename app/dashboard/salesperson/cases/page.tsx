...
export default async function SalespersonCasesPage({ searchParams }: { searchParams: Promise<{ case?: string }> }) {
  ...
  const { data: casesData } = await supabase
    ...
    .select(`
      ...
      employers ( name ),
      job_offers!inner ( assigned_salesperson_id, countries ( name, code ) )
    `)
    .eq('status', 'approved')
    .eq('job_offers.assigned_salesperson_id', user.id)
  ...
  let travel = null;
  if (selected) {
    const { data } = await supabase.from('visa_case_travel').select('*').eq('visa_case_id', selected.id).maybeSingle();
    travel = data;
  }
